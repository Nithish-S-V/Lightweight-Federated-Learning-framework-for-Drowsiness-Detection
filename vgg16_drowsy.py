...
# Load pre-trained VGG-16 (feature extraction layers frozen)
base_model = VGG16(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
base_model.trainable = False  # Freeze layers

# Add custom classification head (as per paper)
...
def train_initial_model():
    ...
    model.compile(loss='binary_crossentropy',  # Match with sigmoid
                  optimizer='adam',
                  metrics=['accuracy'])
   ...
   # Inside train_initial_model() function after training:
    model.save(DATA_DIR.parent / "drowsiness_model_vgg16.keras")  
