...
def send_encrypted_weights(client, encrypted_weights, context):
    ...
    print(f"Number of encrypted weights being sent: {len(encrypted_weights)}")

    # ✅ Remove unnecessary re-encryption, directly serialize
    serialized_weights = [w.serialize() for w in encrypted_weights]
...
def client_socket(base_dir):
    ...
    for set_num in range(1, 6):  # Automate rounds for all 5 sets
        ...
        evaluate_model(model, r"C:\Jeeva\college\sem 8\Major project\new folder 2\test",start_time,end_time,model_size_kb)
        print(f"Model has {len(model.get_weights())} weight tensors")
        encrypted_data = encrypt_weights(model, context)
        send_encrypted_weights(client, encrypted_data, context)
...
if __name__ == "__main__":
    client_socket(r"C:\Jeeva\college\sem 8\Major project\new folder 2\client2")