...
def send_encrypted_weights(client, encrypted_weights):
    """Efficiently serialize and send encrypted weights to the server with chunking and checksum."""
    
    # ✅ Step 1: Validate input
    if not encrypted_weights:
        raise ValueError("❌ Error: No encrypted weights to send!")

    # ✅ Step 2: Serialize weights without large intermediate allocations
    serialized_weights = (struct.pack(">I", len(w.serialize())) + w.serialize() for w in encrypted_weights)


    # ✅ Step 3: Compress the serialized weights efficiently
    compressor = zlib.compressobj()
    compressed_data = b"".join(compressor.compress(data) for data in serialized_weights) + compressor.flush()

    # ✅ Step 4: Compute checksum (SHA-256) to verify integrity
    checksum = hashlib.sha256(compressed_data).digest()

    # ✅ Step 5: Send header (data length + checksum)
    total_size = len(compressed_data)
    header = struct.pack(">I", total_size) + checksum  # (4 bytes length + 32 bytes checksum)

    try:
        client.sendall(header)
        print(f"🔹 Sending encrypted weights (Size: {total_size} bytes, Checksum: {checksum.hex()})")

        # ✅ Step 6: Send data in chunks (e.g., 64KB)
        CHUNK_SIZE = 65536  # 64KB
        sent_bytes = 0
        with tqdm(total=total_size, unit="B", unit_scale=True, desc="Uploading weights") as pbar:
        ...
        print(f"✅ Encrypted weights sent successfully! Total sent: {sent_bytes} bytes")
    
    except Exception as e:
        print(f"❌ Error: Failed to send weights: {e}")
