...
def load_image_data(base_dir, set_num):
    dataset_dir = f"{base_dir}/round{set_num}"
    datagen = tf.keras.preprocessing.image.ImageDataGenerator(rescale=1./255,validation_split=0.2,)  # 20% for validation
...
def client_socket(base_dir):
    ...
    for set_num in range(1, 6):  # Automate rounds for all 5 sets
        ...
        model.fit(train_flow, epochs=10, validation_data=val_flow, verbose = 1)
        import numpy as np

        weights_log_path = "model_weights_client3_log.txt"

        with open(weights_log_path, "a") as f:
            f.write(f"\n===== Round {set_num} Weights =====\n")
            for i, layer in enumerate(model.layers):
                weights = layer.get_weights()
                if weights:  # Some layers may not have weights
                    f.write(f"\n--- Layer {i}: {layer.name} ---\n")
                    for weight_array in weights:
                        np.set_printoptions(threshold=5, edgeitems=2)  # Adjust for readability
                        f.write(np.array2string(weight_array, separator=', ') + '\n')

        # Evaluate the locally trained model
        evaluate_model(model, r"D:\Major Project\Rasp\old\test")
...
if __name__ == "__main__":
    client_socket(r"D:\Major Project\Rasp\old\split_clients_4_with_rounds\client3")