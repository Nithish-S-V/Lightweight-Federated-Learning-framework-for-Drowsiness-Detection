...
# def aggregate_weights(client_weights, past_model_weights, round_index, mode, num_clusters=2):
#     """
#     FedCDA Aggregation:
...
def compute_pairwise_divergence_matrix(client_weights):
    """
    Computes a pairwise cosine distance matrix between client models.
    Input: list of model weights (List of List of np.ndarray)
    Output: 2D np.ndarray (num_clients x num_clients)
    """
    flat_weights = [flatten_model_weights(w) for w in client_weights]
    return cosine_distances(flat_weights)

def cluster_clients_from_divergence(divergence_matrix, num_clusters):
    """
    Clusters clients using agglomerative clustering on divergence matrix.
    """
...
def fed_divergence_aware_aggregate(client_weights, past_model_weights, round_index, num_clusters=2):
    """
    Federated aggregation using divergence-first clustering and temporal smoothing.
    """
    num_clients = len(client_weights)
    if num_clients < num_clusters:
        raise ValueError("Number of clusters cannot exceed number of clients.")
    ...
    divergence_matrix = compute_pairwise_divergence_matrix(client_weights)

    # Step 2: Cluster clients based on divergence
    cluster_labels = cluster_clients_from_divergence(divergence_matrix, num_clusters)
    clusters = {k: [] for k in range(num_clusters)}
    for idx, label in enumerate(cluster_labels):
        clusters[label].append(client_weights[idx])
    ...
    # Step 4: Inter-cluster aggregation (weighted by size)
    total_clients = sum(cluster_sizes)
    aggregated_weights = [
        sum(cluster_aggregates[c][i] * (cluster_sizes[c] / total_clients) for c in range(num_clusters))
        for i in range(num_layers)
    ]
...
train_datagen = tf.keras.preprocessing.image.ImageDataGenerator(rescale=1.0 / 255,validation_split=0.2)  # Split for validation)
...
def server_socket():
    ...
    #distiller = tf.keras.models.load_model(r"D:\Major Project\Rasp\Final_Results_MEtrcs\drowsiness_model_student_Distilled_5_epoch.keras")
    ...
    global_model = teacher_model #use distiller.student when training
    ...
    x = time.time()
    for round_num in range(1, 6):  # Perform 3 rounds
        x_per_round = time.time()
        print(f"\n==== Round {round_num} ====")
        client_weights = []
        ...
        # aggregated_weights = fed_divergence_aware_aggregate(
        #     client_weights,
        ...
        aggregated_weights = fedavg_aggregate(client_weights, [1] * len(client_weights))
        #aggregated_weights = fedtrimmedavg_aggregate(client_weights)
        #aggregated_weights = fedma_aggregate(client_weights)
        #aggregated_weights = fedpa_aggregate(client_weights,  global_weights=global_model.get_weights())
        ...
        evaluate_model(global_model, r"D:\Major Project\Rasp\old\test")  # Removed directory loading
        y_per_round = time.time()
        per_Round__time_taken_Kd = y_per_round-x_per_round
        print(f"Time Taken Per Round : {per_Round__time_taken_Kd:.4f} seconds")
    y =time.time()
    Total_time_taken_Kd = y-x
    print(f"Total Time Taken For Over All FL : {Total_time_taken_Kd:.4f} seconds")
